...
# This script is used to present the localmaster WRS statistics in graphs.
# It sends packet rate evaluated by the localmaster switch to a Graphite
...
# localmaster name (used in metric path)
LMNAME=""
...
FILE_PATH=$MONDATA_DIR/$WRSNAME
if [ -f $FILE_PATH ]; then
        FULLDOMAINNAME=$(cat $FILE_PATH) # nwt0013m66.timing.acc.gsi.de
        LMNAME=${FULLDOMAINNAME%%.*}     # nwt0013m66
else
...
while true; do
	...
	for MONDATA_FILE in "${MONDATA_FILES[@]}"; do
		...
		if [ "$METRIC_VAL" != "" ]; then
			...
			METRIC_KEY=${NWDATA}.${LMNAME}.${DL_NAME}.${PORT_NAME}.${TX_RX}.${METRIC_NAME}  # nw.nwt0013m66.dm.port1.tx.rate10s
